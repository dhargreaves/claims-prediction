...
class TrainingDataLoader(DataLoader):
    ...
    def collect_from_database(self, query: str) -> pd.DataFrame:
        ...
        df = pd.DataFrame(
            features, columns=[f"numeric_{i+1}" for i in range(n_features)]
        )
        ...
        df["income level"] = pd.cut(
            df["income"], bins=5, labels=False, include_lowest=True
        )
    ...
    def load_data(self):
        dataset_from_database = self.collect_from_database(
